...
struct SVMScheduler {
    ...
    SVMScheduler(uint64_t cache_size) : opt_gen_(cache_size), cache_(cache_size) {}
...
struct LRUScheduler {
    LRUScheduler(uint64_t cache_size) {}
};

struct LRUSVMScheduler : Scheduler {
    SVMScheduler svm_;
    SVMScheduler lru_;

    LRUSVMScheduler(uint64_t cache_size) : Scheduler(cache_size), svm_(3 * cache_size / 4), lru_(cache_size / 4) {}
    ...
    Result run(std::vector<Request> &requests) {
        ...
        for (const auto &request: requests) {
            ...
            if (!svm_.cache_.Contains(request.obj_id) && !lru_.cache_.Contains(request.obj_id)) {
                result.cache_misses++;
            }
            int64_t friendliness = svm_.isvm_table_[SVMScheduler::Hash(request.obj_id)].Predict(svm_.pchr_);
            if (friendliness >= 60) {
                friendliness = 2;
            } else if (friendliness >= 0) {
                friendliness = 1;
...
PYBIND11_MODULE(lrusvm, m) {
    py::class_<LRUSVMScheduler>(m, "LRUSVM")
        .def(py::init<const uint64_t>())
        .def("run", &LRUSVMScheduler::run);
}